...
/// <summary>
/// Finds candidate HID device paths for a Pulsar mouse dongle's command channel: vendor 0x3710,
/// usage page 0xFFFF, 65-byte feature reports. Returns every match rather than just the first -
/// on the hardware this was verified against, two identical-looking vendor collections show up
/// (only one of them actually answers; the other fails the command with ERROR_GEN_FAILURE), so
/// the provider tries each in turn instead of hardcoding which interface index is "the" one.
/// </summary>
internal static class PulsarHidLocator
{
    private const ushort PulsarVendorId = 0x3710;
    private const ushort CommandUsagePage = 0xFFFF;
    private const int CommandReportLength = 65;
    ...
    public static IReadOnlyList<string> FindCommandChannelPaths()
    {
        ...
        try
        {
            ...
            for (uint index = 0; SetupDiEnumDeviceInterfaces(deviceInfoSet, IntPtr.Zero, ref hidGuid, index, ref interfaceData); index++)
            {
                ...
                if (path is not null && IsCommandChannel(path))
                {
                    results.Add(path);
                }
            }
    ...
    private static bool IsCommandChannel(string path)
    {
        ...
        if (!HidD_GetAttributes(handle, ref attributes) || attributes.VendorID != PulsarVendorId)
        {
            return false;
        }
        ...
        try
        {
            ...
            return caps.UsagePage == CommandUsagePage && caps.FeatureReportByteLength == CommandReportLength;
        }
        finally
        {
