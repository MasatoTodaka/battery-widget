...
/// <summary>
/// Finds the raw HID device path for the Sony INZONE dongle's control channel: vendor 0x054C,
/// usage page 0xFF04, 64-byte reports. Deliberately does not filter by product id - INZONE ships
/// several dongle PIDs across models (Buds, H9, H9 II, ...) and all of them expose this same
/// collection shape.
/// </summary>
internal static class InzoneHidLocator
{
    private const ushort SonyVendorId = 0x054C;
    private const ushort ControlChannelUsagePage = 0xFF04;
    private const int ControlChannelReportLength = 64;
    ...
    private const int HidpStatusSuccess = 0x00110000; // NTSTATUS success code from hidpi.h's HIDP_ERROR_CODES(0,0)

    public static string? FindControlChannelPath()
    {
    ...
    private static bool IsControlChannel(string path)
    {
        ...
        if (!HidD_GetAttributes(handle, ref attributes) || attributes.VendorID != SonyVendorId)
        {
            return false;
        }
        ...
        try
        {
            ...
            return caps.UsagePage == ControlChannelUsagePage && caps.InputReportByteLength == ControlChannelReportLength;
        }
        finally
        {
    ...
    private static string? GetDevicePath(IntPtr deviceInfoSet, ref SP_DEVICE_INTERFACE_DATA interfaceData)
    {
        ...
        try
        {
            // SP_DEVICE_INTERFACE_DETAIL_DATA.cbSize must be set to the size of the fixed part of
            // the struct (not the whole variable-length buffer) - a well-known Win32 API quirk.
